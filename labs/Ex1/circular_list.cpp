...
Pair GetPointer(List list, int k)
{
    ...
    int i = 1;
    pair.parent = list.head;
    pair.child = list.head->next;
    while (i != k)
    {
...
{
    if (pair.child == pList->tail)
        pList->tail = pair.parent;
    pair.parent->next = pair.child->next;
    free(pair.child);
    pList->size--;
    return true;
}
